...
interface Store {
  ...
  locations: Location[];
  matches: Match[];
  addTeam: (team: Omit<Team, 'id' | 'played' | 'won' | 'drawn' | 'lost' | 'goalsFor' | 'goalsAgainst' | 'points'>) => { id: string };
...
export const useStore = create<Store>((set) => ({
  ...
  clearLeagueFixtures: (leagueId) => set((state) => ({
    matches: state.matches.filter(match => match.leagueId !== leagueId)
  })),

  generateLeagueFixtures: (leagueId) => set((state) => {
